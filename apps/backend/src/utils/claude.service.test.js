...
describe("Claude Service", () => {
  ...
  beforeEach(() => {
    ...
    mockClient = claudeService.client;
  });
  ...
  describe("extractEvent", () => {
    it("deve extrair evento com sucesso", async () => {
      ...
      const result = await claudeService.extractEvent(
        "Próxima segunda 14h reunião com João"
      );
    ...
    it("deve validar datas ISO", async () => {
      ...
      try {
        ...
        expect(error.message).toContain("ISO");
      }
    });
  ...
  describe("extractEventWithTools", () => {
    it("deve extrair evento usando tool use", async () => {
      ...
      const result = await claudeService.extractEventWithTools(
        "Próxima segunda 14h reunião com João"
      );
