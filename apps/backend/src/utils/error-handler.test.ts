...
describe("errorHandler", () => {
  ...
  it("inclui detalhes de validação quando for ValidationError", () => {
    const result = errorHandler(new ValidationError("campos inválidos", [{ field: "email" }]));
    const body = JSON.parse(result.body);
...
function fakeEvent(overrides: Partial<APIGatewayProxyEvent> = {}): APIGatewayProxyEvent {
  return { headers: {}, ...overrides } as APIGatewayProxyEvent;
}
...
describe("wrapAsync", () => {
  ...
  it("usa x-correlation-id do header quando presente", async () => {
    ...
    const result = await handler(fakeEvent({ headers: { "x-correlation-id": "abc-123" } }));
...
describe("errorMiddleware", () => {
  ...
  it("responde 500 genérico para erro desconhecido", () => {
    ...
    expect((res.body as { message: string }).message).toBe("Erro interno do servidor");
  });
});
...
describe("retryWithBackoff", () => {
  ...
  it("tenta novamente até dar certo", async () => {
    const fn = jest.fn().mockRejectedValueOnce(new Error("falha 1")).mockResolvedValue("ok");
