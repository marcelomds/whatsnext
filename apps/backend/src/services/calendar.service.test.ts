...
describe("Calendar Service", () => {
  ...
  describe("createEvent", () => {
    it("cria evento e retorna dados normalizados", async () => {
      ...
      expect(insertArgs.requestBody.reminders.overrides).toEqual([
    ...
    it("usa descrição default quando não informada", async () => {
      ...
      const insertArgs = mockCalendarEvents.insert.mock.calls[0][0];
      expect(insertArgs.requestBody.description).toContain("Criado via WhatsApp");
    });
    ...
    it("envolve erro da API do Google numa mensagem em português", async () => {
      ...
      await expect(service.createEvent({ title: "X", startTime: "t1", endTime: "t2" })).rejects.toThrow(
        "Falha ao criar evento no Google Calendar: insufficient permission"
      );
    });
  });
  ...
  describe("updateEvent", () => {
    it("envia os campos atualizados pro Google Calendar", async () => {
      ...
      expect(mockCalendarEvents.update).toHaveBeenCalledWith(expect.objectContaining({ eventId: "e1" }));
    });
  });
  ...
  describe("listEvents", () => {
    it("normaliza eventos com dateTime", async () => {
      mockCalendarEvents.list.mockResolvedValue({
        data: {
          items: [{ id: "e1", summary: "A", start: { dateTime: "t1" }, end: { dateTime: "t2" } }],
        },
      });
